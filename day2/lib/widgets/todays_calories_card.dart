  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Stack(
        ...
        children: [
          // Lower-Right Orange Line Chart (moved 5px right)
          Positioned(
            right: -11.0,
            bottom: -6.0,
            child: SizedBox(
...
class CaloriesTrendPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 2. Main Line Stroke: exactly 2px with left-to-right gradient:
    // #F8C978 -> #F5B55A -> #EF972D -> #F4B14F (first 35% faded)
    final linePaint = Paint()
      ..shader = ui.Gradient.linear(
        Offset(0, 0),
        Offset(size.width, 0),
        const [
          Color(0x33F8C978), // Faded 35% start
          Color(0xFFF8C978),
          Color(0xFFF5B55A),
          Color(0xFFEF972D),
          Color(0xFFF4B14F),
      ...
      ..style = PaintingStyle.stroke
      ..strokeWidth = 2.0 // Exactly 2px stroke
      ..strokeCap = StrokeCap.round
      ..strokeJoin = StrokeJoin.round;
    canvas.drawPath(path, linePaint);

    // 3. Peak Circle Ring on highest peak (p2)
    // Radius ~3.5px (30% smaller), 2px stroke, increased white center
